...
// ✅ Lazy-load logo only when needed, with a fallback so server never crashes
function getLogoAttachment(): { content: Buffer; cid: string; contentDisposition: 'inline' } | null {
  try {
    const logoPath = path.join(process.cwd(), 'src', 'logo.png');
    const content = fs.readFileSync(logoPath);
    return { content, cid: 'logo', contentDisposition: 'inline' };
  } catch {
    console.warn('Logo not found, sending email without logo');
    return null;
  }
}
...
export async function sendVerificationEmail(to: string, token: string) {
  ...
  const { error } = await resend.emails.send({
    ...
    `),
    attachments: logo
      ? [{ content: logo.content, filename: 'logo.png', headers: { 'Content-ID': '<logo>' } }]
      : [],
  });
